...
class GDLDiffusionTransport(DiffusionTransport):
    ...
    def __init__(self, input_dict: dict,
        ...
        self.diff_coeff_by_length = np.zeros(neumann_shape)
        self.neumann_bc = ls.BoundaryData(values=np.zeros(neumann_shape),
                                          axes=neumann_axes,
                                          indices=neumann_indices)
        dirichlet_axes = self.boundary_patches['Dirichlet']['axes']
        dirichlet_indices = self.boundary_patches['Dirichlet']['indices']
        dirichlet_shape = np.asarray(
            [mf.get_axis_values(conc, dirichlet_axes, dirichlet_indices)
             for i, conc in enumerate(self.concentrations)
             if i != self.id_inert]).shape
        self.dirichlet_bc = ls.BoundaryData(values=np.zeros(dirichlet_shape),
                                            axes=dirichlet_axes,
                                            indices=dirichlet_indices)
    ...
    def calc_flux_scaling_factors(
        ...
        flux_boundary_flux = np.asarray([
            mf.get_axis_values(flux, neumann_bc_axes, neumann_bc_indices)
            for flux in neumann_bc_flux])
        delta_conc = channel_concentration - flux_boundary_concentrations
        flux_scaling_factors = (delta_conc / channel_concentration)
...
class GDLLiquidTransport(GDLDiffusionTransport):
    """
    Class to describe the diffusion in a porous layer (here defined through the
    SolidLayer class
    """
